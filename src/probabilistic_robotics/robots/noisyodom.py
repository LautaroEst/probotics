...
class NoisyOdometryRobot(Robot):
    ...
    def apply_movement(self, r1, t, r2):
        ...
        # noise sigma for r1 
        sigma_r1 = self.noise_params[0] * abs(r1)  + self.noise_params[1] * abs(t)
        r1_noisy = r1 + self._rs.randn() * sigma_r1

        # noise sigma for translation
        sigma_t = self.noise_params[2] * abs(t)  + self.noise_params[3] * abs(r1+r2)
        t_noisy = t + self._rs.randn() * sigma_t

        # noise sigma for r2
        sigma_r2 = self.noise_params[0] * abs(r2)  + self.noise_params[1] * abs(t)
