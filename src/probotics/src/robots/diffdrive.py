...
class DiffDriveRobot(Robot):
    ...
    def __init__(self, initial_pose, radius, wheels_radius, wheels_distance, alpha, seed=None):
    ...
    def diffdrive(self, x, y, theta, vL, vR, dt):
        w = (vR - vL) / self.wheels_distance # Velocidad angular 
        if w == 0:
            v = vR
            ...
            R = self.wheels_distance / 2 * (vL + vR) / (vR - vL) # distancia del ICC al robot
...
class NoisyDiffDriveRobot(DiffDriveRobot):
    ...
    def inverse_kinematics(self, v, w):
        ...
        wL = (v - w * self.wheels_distance / 2) / self.wheels_radius
        wR = (v + w * self.wheels_distance / 2) / self.wheels_radius

        # Velocidades con ruido
        v = self.wheels_radius * (wR + wL) / 2 + self._rs.randn() * self.alpha * (wL**2 + wR**2)
        w = self.wheels_radius * (wR - wL) / self.wheels_distance + self._rs.randn() * self.alpha * (wL**2 + wR**2)
        vL = v - w * self.wheels_distance / 2
        vR = v + w * self.wheels_distance / 2
